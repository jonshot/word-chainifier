      ...
      return new Promise(function (fulfill) {
        ...
        _getDictionary(words).then(function (dictionary) {
          ...
          var checkChains = function (word, target) {
            ...
            if(typeof startChain === 'undefined' || typeof endChain === 'undefined') {
              return false;
            }

            if (resolved || word === target || _.intersection(startChain.getChain(), endChain.getChain()).length) {
          ...
          var built = function () {
            var chain = _.union(startChain.getChain(words.reversed), endChain.getChain(words.reversed));
            var response = _getResponse();
            response.success = true;
...
//                }
                ...
                if (checkChains(curChain.word, targetWord)) {
                  queue.kill();
                  built();
                  callback();
                  return;
